...
@app.callback(
    [Output('user-count-bar', 'figure'),
     ...
     Output('average-age-by-country-map', 'figure'),
     Output('email-domains-pie', 'figure')],  # Most Common Email Domains
...
def update_graphs(n_clicks):
    ...
    user_count_fig = px.bar(df, x='nationality', y='user_count', labels={'user_count': 'User Count'})
    ...
    avg_age_bar_fig = px.bar(df, x='nationality', y='avg_age', labels={'avg_age': 'Average Age'})
    ...
    data_country = list(collection.aggregate(pipeline_country))
    df_country = pd.DataFrame(data_country)

    avg_age_by_country_map = px.choropleth(df_country, 
                                           locations="country", 
    ...
    # For the email domains, create an empty pie chart (null graph)
    email_domains_fig = px.pie(names=["No Data"], values=[1])
